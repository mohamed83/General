...
for line in f:
    ...
    freqShortURLs.append(url)
f.close()
freqShortURLs= freqShortURLs[:40000]
futures = []
responses = []
for u in freqShortURLs:
    f = session.get(u)
    futures.append(f)

print "Requests started: ", len(futures)
logging.info("Requests started", len(futures))
        ...
        if ori_url != "":
            ...
            if ori_url in expanded_url_dict:
                ...
                if len(page) > 10:
                    ...
                    #text = title + " " + text
                    webpages.append((ori_url,title, text))
            i+=1
            if i%1000 == 0:
