...
query = "select count(*), from_user as user_name from twitter." + CONST_TABLE +" where text not like 'RT%' "+ " group by user_name"
#query = "select count(*), from_unixtime(time, '%Y-%m') as fullDate from twapperkeeper_2." + CONST_TABLE + " group by fullDate"
#query = "select count(*), from_unixtime(time, '%Y-%m-%d') as fullDate from twapperkeeper_2." + CONST_TABLE + " group by fullDate"
...
for row in cursor.fetchall():
	#f1.write("%s,%s\n" % (row[1],row[0]))
	users.append((row[1],row[0]))
	
sorted_users = utils.getSorted(users,1)
